...
class TaskHandler(BufferingHandler):
    ...
    def flush(self):
        import traceback
        stdout_print('flush', 0)#, traceback.print_stack())
        if self._task_id is None:
            return
        stdout_print('flush', 1)
        self.counter = 0
        stdout_print('flush', 2)
        if self._background_log:
            stdout_print('flush', 3)
            self._background_log.flush()
            self._background_log_size = 0
            stdout_print('flush', 4)

    def close(self, wait=False):
        stdout_print('close', 0)
        ...
        if _background_log:
            if not _background_log.is_subprocess() or _background_log.is_alive():
                ...
                if wait:
                    # noinspection PyBroadException
                    try:
                        stdout_print('timeout', _background_log.empty())
